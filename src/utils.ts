...
export function importAllItems(rarity: number = 0) {
  const filesNames = [common, uncommon, rare, epic, legendary, mythic, secret];
...
export function getAllPossibleToGetItems(
  player: Player,
  rarity: number,
  action?: string,
  equipped?: Item
): Item[] {
  const items: Item[] = importAllItems(rarity);
  const available = [];
...
export function printWithRarityColor(message: string, rarity: number) {
  switch (rarity) {
    default:
    case 0:
      return message;
...
export function getTimeEmoji() {
  return getCurrentDayPart() == 3 ? Moon.lunarPhaseEmoji() : '☀️';
}
...
export function getCurrentDayPart() {
  ...
  const morningElapsed = season == 'Été' ? 360 : season == 'Hiver' ? 450 : 390;
  const nightElapsed = season == 'Été' ? 1380 : season == 'Hiver' ? 1080 : 1200;
